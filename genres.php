...
<div class="column left">
    <div class="top_rated_directors">
        <h2>Most Movies-Country Per Genre</h2>
        <?php
        require_once 'include/dbConnect.php';
        require_once 'include/functions.php';
        session_start();

        $result = get_genre_country($conn);
    ...
    <div class="top_avg_rated_actors">
        <h2>Most Used Languages Per Genre</h2>
        <?php
        require_once 'include/dbConnect.php';
        require_once 'include/functions.php';

        $result = get_genre_language($conn);
    ...
    <div class="top_rated_genres">
        <h2>Longest Genres</h2>
        <?php
        require_once 'include/dbConnect.php';
        require_once 'include/functions.php';

        $result = get_avg_duration_genre($conn);

        echo "<table>";
        foreach ($result as $row) {
            echo "<tr>";
            echo "<td>| {$row['genre']} </td>";
            printf( "<td>| %.5s </td>",$row['avg']);
            echo "</tr>";
        }
        echo "</table>";
        mysqli_free_result($result);
